...
export async function PATCH(
 ...
 const authToken = await getAuthToken(request)
  ...
  const { creationIds } = requestJson as CollectionsV2RemoveCreationsArguments
  ...
  try {
    ...
    const res = await eden.collections.creations.removeV2({
      collectionId: id,
      creationIds,
    })
    ...
    return NextResponse.json<CollectionsV2RemoveCreationsResponse>(res)
  } catch (error) {
